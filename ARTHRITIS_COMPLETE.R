...
BRFSS_ARTHRITIS_train<- BRFSS_ARTHRITIS[ind==1, ]
dim(BRFSS_ARTHRITIS_train)

BRFSS_ARTHRITIS_test<- BRFSS_ARTHRITIS[ind==2, ]
dim(BRFSS_ARTHRITIS_test)

options("scipen"=100, "digits"=5)

#Proportion of indivisuals who purchased organic products
print(nrow(BRFSS_ARTHRITIS[BRFSS_ARTHRITIS$HAVARTH3=="Yes",])/nrow(BRFSS_ARTHRITIS))
#In percentage
print((nrow(BRFSS_ARTHRITIS[BRFSS_ARTHRITIS$HAVARTH3=="Yes",])*100)/nrow(BRFSS_ARTHRITIS))


#Build decision tree
BRFSS_ARTHRITIS_tree <- rpart(HAVARTH3 ~ ., data = BRFSS_ARTHRITIS_train, method = "class", control = rpart.control(cp = 0.00001, maxdepth=5))

BRFSS_ARTHRITIS_train$pred<-predict(BRFSS_ARTHRITIS_tree,BRFSS_ARTHRITIS_train,type="class")
...
testModelPerformance <- function(model, dataset, target, prediction) {
  ...
  Performancetrain <- testModelPerformance(BRFSS_ARTHRITIS_tree, BRFSS_ARTHRITIS_train, BRFSS_ARTHRITIS_train$HAVARTH3)
  
  writeLines("PERFORMANCE EVALUATION FOR")
  writeLines(paste("Model:", deparse(substitute(BRFSS_ARTHRITIS_tree))))
  writeLines(paste("Target:", deparse(substitute(BRFSS_ARTHRITIS_train))))
  
  writeLines("\n\nConfusion Matrix:")
  confMatrix <- table(Actual = BRFSS_ARTHRITIS_train$HAVARTH3, Predicted = BRFSS_ARTHRITIS_train$pred)
  ...
  BRFSS_ARTHRITIS_test$pred<-predict(BRFSS_ARTHRITIS_tree,BRFSS_ARTHRITIS_test,type="class")
  Performancetest <- testModelPerformance(BRFSS_ARTHRITIS_tree, BRFSS_ARTHRITIS_test, BRFSS_ARTHRITIS_test$HAVARTH3)
  
  writeLines("PERFORMANCE EVALUATION FOR")
  writeLines(paste("Model:", deparse(substitute(BRFSS_ARTHRITIS_tree))))
  writeLines(paste("Target:", deparse(substitute(BRFSS_ARTHRITIS_test))))
  
  writeLines("\n\nConfusion Matrix:")
  confMatrix <- table(Actual = BRFSS_ARTHRITIS_test$HAVARTH3, Predicted = BRFSS_ARTHRITIS_test$pred)
  truePos <- confMatrix[2,2]
  falseNeg <- confMatrix[2,1]
  falsePos <- confMatrix[1,2]
  trueNeg <- confMatrix[1,1]
  ...
  BRFSS_ARTHRITIS_train$correct <- BRFSS_ARTHRITIS_train$HAVARTH3 == BRFSS_ARTHRITIS_train$pred #create a new colum, TRUE if predicted = actual, otherwise FALSE
  traincorrectcount <- length(which(BRFSS_ARTHRITIS_train$correct))
  trainincorrectcount <- nrow(BRFSS_ARTHRITIS_train) - traincorrectcount
  trainerrorrate <- trainincorrectcount/nrow(BRFSS_ARTHRITIS_train)
  trainaccuracy <- 1-trainerrorrate
  
  BRFSS_ARTHRITIS_test$correct <- BRFSS_ARTHRITIS_test$HAVARTH3 == BRFSS_ARTHRITIS_test$pred #create a new colum, TRUE if predicted = actual, otherwise FALSE
  testcorrectcount <- length(which(BRFSS_ARTHRITIS_test$correct))
  testincorrectcount <- nrow(BRFSS_ARTHRITIS_test) - testcorrectcount
  testerrorrate <- testincorrectcount/nrow(BRFSS_ARTHRITIS_test)
  ...
  rpart.plot(BRFSS_ARTHRITIS_tree)
  
  BRFSS_ARTHRITIS_test$HAVARTH3<-ifelse(BRFSS_ARTHRITIS_test$HAVARTH3=="Yes",1,2)
  BRFSS_ARTHRITIS_test$pred<-ifelse(BRFSS_ARTHRITIS_test$pred=="Yes",1,2)                      
 
 library(gains)
 #Evaluate model performance on test data
gain <- gains(BRFSS_ARTHRITIS_test$HAVARTH3, BRFSS_ARTHRITIS_test$pred, groups=length(BRFSS_ARTHRITIS_test$pred))
 
  #plot lift chart
plot(c(0,gain$cume.pct.of.total*sum(BRFSS_ARTHRITIS_test$HAVARTH3))~c(0,gain$cume.obs),
     xlab="# cases", ylab="Cumulative", main="", type="l")
lines(c(0,sum(BRFSS_ARTHRITIS_test$HAVARTH3))~c(0, dim(BRFSS_ARTHRITIS_test)[1]), lty=2)
# compute deciles and plot decile-wise chart
heights <- gain$mean.resp/mean(BRFSS_ARTHRITIS_test$HAVARTH3)
